...
static llvm_value_t llvm_emit_ast_expr(ast_expr_t* expr, var2reg_map_t* var2reg, llvm_function_t* f){
	switch (expr->type){
		case AST_EXPR_CONST:
			return (llvm_value_t){ .type=LLVM_VALUE_INT_CONST, .int_const=expr->constant.value };
		case AST_EXPR_VAR_REF:
			...
			return (llvm_value_t){
					.type = LLVM_VALUE_REG,
					.reg.reg = reg,
					.reg.ast_type = expr->var_ref->type_ref,
				};
		...
		case AST_EXPR_UNOP:
			...
			return (llvm_value_t){ .type=LLVM_VALUE_REG, .reg.reg=llvm_add_inst(f, inst), .reg.ast_type=0 }; // TODO: assign ast type, assign llvm type
